...
## 2026-06-17 - Path Bypass via Regional and Script-Specific Homoglyphs
...
## 2026-06-18 - Path Bypass via Astral Plane Word Separators
**Vulnerability:** Open redirect and protocol-relative URL bypass via obscure Unicode word separators from ancient scripts (e.g., Carian, Anatolian, Cuneiform).
**Learning:** Obscure Unicode characters such as Carian Word Separator Line (U+10290), Anatolian Hieroglyph Word Divider (U+145B1), and Cuneiform Punctuation Word Divider (U+12470) are visually similar to slashes or dots and can be interpreted as path delimiters by some browsers or downstream systems. Furthermore, when using these astral plane characters in JavaScript regular expressions, the `u` (Unicode) flag is required for correct interpretation of `\u{XXXXX}` escapes; without it, the regex engine may fail to match or interpret the sequence incorrectly.
**Prevention:** Continuously update relative path blocklists in URL sanitizers to include word dividers from ancient and script-specific blocks (U+10290, U+10291, U+145B1, U+103D0, U+1039F, U+1091F, U+1029F, U+1093F, U+109BE, U+109BF, U+10A7F, U+10AF6, U+10B39, U+10B3A, U+10E7E, U+10A9F, U+12470). Always use the `u` flag in security-critical regexes.
