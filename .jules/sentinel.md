...
## 2026-06-18 - Path Bypass via Ancient Script Word Dividers
**Vulnerability:** Open redirect and path traversal bypass via script-specific word dividers from Carian, Anatolian, Lycian, and other ancient blocks.
**Learning:** Obscure punctuation marks from ancient script blocks (e.g., Carian Word Separator Line U+10290, Anatolian Hieroglyph Word Divider U+145B1, Cuneiform Punctuation Divider U+12470) can be interpreted as dots or slashes by various components in the web stack. Because these characters are in the astral plane (non-BMP), they are often missed by standard security filters or incorrect regex patterns.
**Prevention:** Relative path blocklists must comprehensively include astral plane homoglyphs. When using JavaScript regular expressions, ensure the 'u' flag is enabled and non-BMP characters are correctly represented (e.g., `\u{10290}`). Always verify coverage with reproduction scripts targeting specific Unicode blocks.
