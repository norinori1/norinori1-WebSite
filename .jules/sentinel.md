...
## 2025-05-25 - Advanced URL Hardening and Host Normalization
**Vulnerability:** Potential security bypasses via unnormalized hostnames, embedded credentials, and malicious Unicode characters.
**Learning:** Hostname-based security checks can be bypassed using trailing dots (e.g., `notion.so.`), which many browsers and systems treat as the same host. Additionally, absolute URLs can contain embedded credentials (`user:pass@host`) that might leak if not explicitly stripped. Certain Unicode characters like BiDi isolates (U+2066-U+2069) and line/paragraph separators (U+2028-U+2029) can be used for UI spoofing or protocol obfuscation.
**Prevention:** Always normalize hostnames by stripping trailing dots before validation. Explicitly strip credentials (username and password) from URLs using the `URL` constructor during sanitization. Use `NFC` normalization and expand regex filters to include all known dangerous Unicode characters (BiDi isolates, separators, and zero-width characters).
